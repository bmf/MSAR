...
# New Tasks Added for PRD v3 (Do not alter completed items above)

> These tasks extend the scope to support **multi-contracts**, **PWS line items lifecycle**, **multi‑assignee tasks**, **lead approval → report queue → PM/APM approval**, **PDF exports**, and **role/contract-scoped RLS** per the updated PRD.

## Phase 1 — Supabase Setup & Data Model (Additions)

- [ ] **1.5 Create Contract-Scoped Core Tables**
...
- [ ] **1.8 Storage Bucket for Reports**
  - *Acceptance Criteria:* Supabase Storage bucket (e.g., `reports`) created with folder structure `reports/{CONTRACT_CODE}/{YYYY-MM}/report.pdf`. Public access disabled; signed URL retrieval works in dev.
...
## Phase 5 — Task Update Workflow (Additions)
...
## Phase 6 — Team Lead Review (Additions)
...
## Phase 7 — PM/APM Reporting & PDF Export (Additions)
...
## Phase 8 — Admin: User, Contracts, Teams, PWS (Additions)
...
## Phase 9 — Non-Functional (Additions)
...
## Phase 10 — DevOps & CI/CD (Additions)
...
## Phase 11 — QA: Playwright (Additions)
...
## Phase 12 — Documentation & Handover (Additions)
